view: centpark_lights {
  sql_table_name: centpark_lights ;;
...
### FieldUnits.Alex Pump.Meters.MWM BTHS DRV ALEX PMP.Flow
  ...
  dimension: time_8am_8am {
    label: "time 8am - 8am"
    type:string
    sql:(trunc_days(${centpark_lights.reading_time}) = add_days(-1,trunc_days(now())) AND extract_hours(${centpark_lights.reading_time}) >= 8) OR
      (trunc_days(${centpark_lights.reading_time}) = trunc_days(now()) AND extract_hours(${centpark_lights.reading_time}) < 8);;
  }
